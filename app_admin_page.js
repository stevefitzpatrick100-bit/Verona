...
//
// Navigation model:
//   Adoption (users table)
//     → User journey (one user's sessions)
//       → Session analysis (one conversation + extracts)
//
// Breadcrumb at the top navigates between them.
// Invites tab is separate.
// =====================================================================
...
function durationMin(startedAt, endedAt) {
  ...
  const mins = Math.round((end - new Date(startedAt)) / 60000);
  return mins;
}
...
export default function Admin() {
  ...
  const [tab, setTab] = useState("conversations"); // conversations | invites
...
function Adoption({ data, onSelectUser }) {
  ...
  const rows = useMemo(() => {
    return data.users.map((u) => {
      ...
      const lastSession = userSessions[0]; // sessions are desc by started_at
      const lastActive = lastSession?.started_at;
      const totalMins = userSessions.reduce((acc, s) => {
        const m = durationMin(s.started_at, s.ended_at);
        return acc + (m || 0);
      }, 0);
      const currentPhase = lastSession?.phase || "—";
      return {
        id: u.id,
  ...
  if (rows.length === 0) {
    return (
      <div style={{ padding: 40, textAlign: "center", color: "#888" }}>
        No users yet. Create an invite and share the link.
      </div>
    );
  }
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={S.tableWrap}>
          <div style={{ ...S.tableRow, ...S.tableHead }}>
            ...
            <div style={{ flex: 1.5 }}>Joined</div>
            <div style={{ flex: 1.5 }}>Last active</div>
            <div style={{ flex: 1, textAlign: "right" }}>Sessions</div>
            <div style={{ flex: 1, textAlign: "right" }}>Time</div>
            <div style={{ flex: 1.2 }}>Phase</div>
          </div>
          {rows.map((r) => (
            <div key={r.id} style={S.tableRow} onClick={() => onSelectUser(r.id)} className="va-row">
              ...
              <div style={{ flex: 1.5, color: "#aaa", fontSize: 12 }}>{formatDate(r.joined)}</div>
              <div style={{ flex: 1.5, color: "#aaa", fontSize: 12 }}>{timeAgo(r.lastActive)}</div>
              <div style={{ flex: 1, textAlign: "right", fontWeight: 500 }}>{r.sessionsCount}</div>
              <div style={{ flex: 1, textAlign: "right", color: "#aaa", fontSize: 12 }}>
                {r.totalMins ? `${r.totalMins}m` : "—"}
              </div>
              <div style={{ flex: 1.2, color: "#C4A08A", fontSize: 12, textTransform: "capitalize" }}>{r.currentPhase}</div>
            </div>
          ))}
        </div>
      </div>
...
function UserJourney({ data, userId, onSelectSession }) {
  ...
  const userTopDimensions = data.dimensions
      ...
      <div style={S.userHeader}>
        <div>
          ...
          <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
            Joined {formatDate(user.created_at)} · {userSessions.length} session{userSessions.length === 1 ? "" : "s"}
        ...
        <div>
          ...
          {userEssentialTruth?.text && (
            <div style={S.portraitSection}>
              ...
              <div style={{ fontSize: 13, fontStyle: "italic", lineHeight: 1.5, color: "#e0ddd8" }}>
                "{userEssentialTruth.text}"
              </div>
              <div style={{ fontSize: 10, color: "#666", marginTop: 4 }}>
                Confidence: {userEssentialTruth.confidence}/5
              </div>
            </div>
          )}
          ...
          {userActiveHypotheses.length > 0 && (
            <div style={S.portraitSection}>
              ...
              {userActiveHypotheses.slice(0, 5).map((h) => (
                <div key={h.id} style={S.hypothesisChip}>
                  {h.text}
                </div>
              ))}
            </div>
...
function SessionCard({ session, data, onClick }) {
  ...
  // Latest scores during this session
    ...
    <div style={S.sessionCard} onClick={onClick} className="va-card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
        <div style={{ fontSize: 14, fontWeight: 500 }}>
          ...
          <span style={{ color: "#C4A08A", fontSize: 12, fontWeight: 400, marginLeft: 10 }}>
            {session.phase}
          </span>
        </div>
      ...
      {(sessionFragments.length > 0 || sessionHypotheses.length > 0 || sessionKeyMoments.length > 0 || sessionTerritoryUpdates.length > 0 || sessionDimsEvidenced.length > 0) && (
        <div style={{ marginBottom: 10 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 18, fontSize: 12, lineHeight: 1.6, color: "#d0cdc8" }}>
            {sessionFragments.slice(0, 3).map((f) => (
            ...
            {sessionHypotheses.map((h) => (
              <li key={h.id}>
                Hypothesis {h.created_session === session.session_number ? "added" : h.status}: {h.text}
              </li>
            ))}
            {sessionKeyMoments.map((k) => (
              <li key={k.id}>
                <span style={{ color: "#C4A08A" }}>Key moment ({k.moment_type}):</span> {k.description}
              </li>
            ))}
            {sessionTerritoryUpdates.map((t) => (
              <li key={t.id}>Territory: {t.territory} (depth {t.depth}/5)</li>
            ))}
            {sessionDimsEvidenced.length > 0 && (
              <li style={{ color: "#888" }}>{sessionDimsEvidenced.length} dimension{sessionDimsEvidenced.length === 1 ? "" : "s"} evidenced</li>
            )}
          </ul>
        </div>
      )}
      ...
      {!latestScore && (
        <div style={{ fontSize: 11, color: "#C4A08A", textAlign: "right", paddingTop: 8, borderTop: "1px solid #2a2a2a" }}>
          View conversation →
        </div>
      )}
...
function SessionAnalysis({ data, userId, sessionId }) {
  ...
  const sessionDimsEvidenced = data.dimensions
    .filter((d) => d.user_id === userId && d.last_evidence_session === session.session_number)
    .sort((a, b) => (b.weight || 0) - (a.weight || 0));
    ...
    <div style={{ padding: "20px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 24, marginTop: 20 }}>
            ...
            {messages.map((m) => {
              ...
              return (
                <div key={m.id} style={{
                  ...
                  <div style={{ fontSize: 13, lineHeight: 1.6, whiteSpace: "pre-wrap", color: "#e0ddd8" }}>
                    {m.content}
                  </div>
                </div>
              );
        ...
        <div>
          ...
          {lastScore && (
            <div style={S.portraitSection}>
              <div style={S.portraitLabel}>
                Scores {firstScore !== lastScore ? "(at session end)" : ""}
              </div>
              {[
            ...
            <div style={S.portraitSection}>
              ...
              {sessionKeyMoments.map((k) => (
                <div key={k.id} style={S.analysisChip}>
                  <span style={{ color: "#C4A08A", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", marginRight: 6 }}>
                    {k.moment_type || "moment"}
                  </span>
                  {k.description}
                  {k.portrait_impact && (
                    <div style={{ fontSize: 10, color: "#666", marginTop: 4, fontStyle: "italic" }}>
                      Impact: {k.portrait_impact}
                    </div>
                  )}
                </div>
              ))}
          ...
          {sessionHypotheses.length > 0 && (
            <div style={S.portraitSection}>
              ...
              {sessionHypotheses.map((h) => {
                ...
                const isResolvedThisSession = h.resolved_session === session.session_number && !isNewThisSession;
                return (
              ...
              {sessionDimsEvidenced.slice(0, 10).map((d) => (
                <div key={d.id} style={S.dimRow}>
                  ...
                  <span style={{ fontSize: 10, color: "#888" }}>
                    {d.resolution} · conf {d.confidence}
                  </span>
                </div>
              ))}
              {sessionDimsEvidenced.length > 10 && (
                <div style={{ fontSize: 10, color: "#666", marginTop: 4 }}>+{sessionDimsEvidenced.length - 10} more</div>
              )}
            </div>
          )}
...
function Invites({ invites, newInviteName, setNewInviteName, createInvite, deleteInvite }) {
  return (
    <div style={{ padding: "20px 24px", maxWidth: 900 }}>
      ...
      {invites.map((inv) => (
        <div key={inv.id} style={S.inviteRow}>
          ...
          />
          <button onClick={() => deleteInvite(inv.id)} style={{ ...S.btn, background: "#433", color: "#E24B4A", fontSize: 11 }}>
            Delete
          </button>
        </div>
      ))}
