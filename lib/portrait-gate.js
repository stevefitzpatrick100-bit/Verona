...
export async function canWritePortrait(supabase, userId) {
  ...
  const { data: latestCq } = await supabase
    .from("cq_dimensions")
    .select("dependency_risk, agency, frustration, room, measured_at")
    .eq("user_id", userId)
  ...
  if (latestCq) {
    ...
    const agency = latestCq.agency ?? 10;
    if (dep > 6 || agency < 4 || frustrationSpikes >= 2) {
      return {
        ready: false,
        missing: GATE_MISSING_RESOLUTION,
        reason: "observer_signal_indicates_unmetabolised_state",
        what_is_needed: "time_in_therapy_room",
        signal: { dependency_risk: dep, agency, frustration_spikes: frustrationSpikes },
      };
    }
  }
