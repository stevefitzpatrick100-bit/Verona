...
export async function applyPortraitUpdate(supabase, userId, sessionNumber, analysis) {
  ...
  if (analysis.dimensions) {
    for (const [name, data] of Object.entries(analysis.dimensions)) {
      const stated = data.stated != null ? parseFloat(data.stated) : null;
      const revealed = data.revealed != null ? parseFloat(data.revealed) : null;
      promises.push(
        supabase.from("portrait_dimensions").upsert({
          user_id: userId,
          dimension_name: name,
          grouping: data.grouping || "uncategorised",
          position_stated: isFinite(stated) ? stated : null,
          position_revealed: isFinite(revealed) ? revealed : null,
  ...
  if (analysis.partner_image) {
    for (const [name, data] of Object.entries(analysis.partner_image)) {
      const stated = data.stated != null ? parseFloat(data.stated) : null;
      const revealed = data.revealed != null ? parseFloat(data.revealed) : null;
      const grouping = data.grouping || data.category || "uncategorised";
      promises.push(
        supabase.from("partner_dimensions").upsert({
          user_id: userId,
          dimension_name: name,
          category: grouping, // legacy NOT NULL
          grouping,
          position_stated: isFinite(stated) ? stated : null,
          position_revealed: isFinite(revealed) ? revealed : null,
          evidence: data.evidence || null,
  ...
  if (analysis.relationship_image) {
    for (const [name, data] of Object.entries(analysis.relationship_image)) {
      const stated = data.stated != null ? parseFloat(data.stated) : null;
      const revealed = data.revealed != null ? parseFloat(data.revealed) : null;
      const tierVal = Number.isInteger(data.tier) && data.tier >= 1 && data.tier <= 3 ? data.tier : 2;
      promises.push(
        supabase.from("relationship_dimensions").upsert({
          user_id: userId,
          dimension_name: name,
          tier: tierVal, // legacy NOT NULL
          grouping: data.grouping || "uncategorised",
          position_stated: isFinite(stated) ? stated : null,
          position_revealed: isFinite(revealed) ? revealed : null,
          evidence: data.evidence || null,
