...
export async function buildPortraitContext(supabase, userId) {
  ...
  if (relDims?.length) {
    ...
    for (const r of relDims) {
      const pos = r.evidenced_position || r.imagined_position || "?";
      ctx += `- ${r.dimension_name} [${r.grouping}, tier ${r.tier}]: desired_overlap=${pos}, flexibility=${r.flexibility} (${r.resolution})\n`;
    }
    ctx += "\n";
  }
