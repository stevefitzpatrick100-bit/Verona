...
export async function buildPortraitContext(supabase, userId) {
  ...
  if (sessionCount <= 1) {
    // Still fetch user details so Angelica can greet by name
    ...
    let firstCtx = "This is your first conversation with this person.";
    if (user?.display_name) {
      firstCtx += ` Their name is ${user.display_name}.`;
    }
    if (user?.invited_by_name) {
      firstCtx += ` They were introduced by ${user.invited_by_name}.`;
    }
    firstCtx += " You know nothing else about them yet. Greet them warmly by name.";
    return firstCtx;
  }
  ...
  if (dims?.length) {
    ...
    for (const d of dims) {
      ...
      ctx += `stated=${stated ?? "?"}, revealed=${revealed ?? "?"}`;
      ctx += ` (${d.resolution}, conf ${d.confidence})`;
      if (d.evidence) ctx += ` — ${d.evidence}`;
      ctx += "\n";
    }
    ctx += "\n";
  }
  ...
  if (partner?.length) {
    ...
    for (const p of partner) {
      const stated = p.position_stated;
      const revealed = p.position_revealed ?? p.position;
      const note = p.evidence || p.tested_value || p.inferred_value || p.stated_value || "";
      ctx += `- ${p.dimension_name} [${p.category || p.grouping || "uncategorised"}]: stated=${stated ?? "?"}, revealed=${revealed ?? "?"} (${p.flexibility})`;
      if (note) ctx += ` — ${note}`;
      ctx += "\n";
    }
    ctx += "\n";
  ...
  // Get relationship dimensions (non-unvisited)
  const { data: relDims } = await supabase
  ...
  if (relDims?.length) {
    ctx += `## Relationship Shape Preferences\n`;
    for (const r of relDims) {
      const stated = r.position_stated ?? r.imagined_position;
      const revealed = r.position_revealed ?? r.evidenced_position;
      const w = r.weight ?? r.user_weight ?? r.default_weight;
      ctx += `- ${r.dimension_name} [${r.grouping}, w${w ?? "?"}]: stated=${stated ?? "?"}, revealed=${revealed ?? "?"}, flexibility=${r.flexibility} (${r.resolution})`;
      if (r.evidence) ctx += ` — ${r.evidence}`;
      ctx += "\n";
    }
    ctx += "\n";
  }

  // Get meta layer (trust calibration)
  ...
  if (meta) {
    ctx += `## Trust Calibration\n`;
    ctx += `trust_in_ai: ${meta.trust_in_ai}/10, consistency: ${meta.consistency}/10, openness: ${meta.openness}/10\n`;
    ctx += `emotional_availability: ${meta.emotional_availability}/10, readiness: ${meta.readiness}/10\n`;
    if (meta.gap_direction) ctx += `stated_vs_revealed gap: ${meta.gap_direction}\n`;
    ctx += "\n";
  }
