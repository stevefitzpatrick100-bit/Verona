-- 010_dimension_catalogue.sql
-- Reference table: the canonical 100 portrait dimensions.
-- Copied from Files/New dimensions/dimension_catalogue.sql.
-- Single source of truth for dimension definitions; per-user
-- portrait_dimensions rows reference these by name.

create table if not exists dimension_catalogue (
  dimension_name text primary key,
  display_order int not null unique,
  grouping text not null,
  scale_low text not null,
  scale_high text not null,
...
create index if not exists idx_catalogue_grouping on dimension_catalogue(grouping, display_order);
create index if not exists idx_catalogue_weight on dimension_catalogue(default_weight desc);

-- Seed only if empty so re-running is safe.
do $$
begin
  if (select count(*) from dimension_catalogue) > 0 then
    return;
  end if;
...
-- VERIFY: select count(*) from dimension_catalogue;  -- expect 100
